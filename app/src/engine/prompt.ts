...
export function buildRateSystem(s: Scenario): string {
  return [
    ...
    `- "offer"    — the speaker gives something real of THEIR OWN that COSTS them to say it: a confession,`,
    `               an admission that exposes them, a vulnerability, a loss they carry, a real risk taken`,
    `               by speaking — a genuine step in the currency above. TWO tests, BOTH required: (1) the`,
    `               give is CONCRETE — a named event, deed, loss, or lived detail of the speaker's own`,
    `               life; AND (2) saying it COSTS the speaker — it exposes, incriminates, or leaves them`,
    `               open. Concrete alone is NOT a give: fond reminiscing about their own past, trade, or`,
    `               craft that risks nothing (how fine the goods were, the good years of the work) is`,
    `               atmosphere — warm, but it costs them nothing, so it is never "offer".`,
    ...
    `  "I still recall how fine the silk felt, the good years when the work came easy" → probe   (fond reminiscing of their own past; concrete but risks nothing — a give must cost the speaker)`,
